...
static void startNetworkThread(TCPConnector *httpArgs)
{
	...
	if (httd_handle != LWP_THREAD_NULL && httpArgs->threadActive == 1)
	{
		if (httpArgs->waitingForServer)
		{
			...
			LWP_SuspendThread(httd_handle);
			usleep(10000);
			net_close (httpArgs->sock);
			usleep(10000);
			httpArgs->requestStop = 1;
			LWP_ResumeThread(httd_handle);
			LWP_JoinThread(httd_handle, NULL);
    ...
    {
		LOG_AS("Network configuration failed! Error code %i\n", ret);
		httpArgs->internalState = CONNECTION_ERROR_NO_NETWORK_DEVICE;
	}
}

void handeConnectionLoop(u8 port)
{
	LOG_N("\nStarting GBA Connection\n");

	SerialConnector connector;
	connector.gcport = port;
	...
	tcpConnector.remoteAddressAndPort = "127.10.0.1:9000";
    ...
    while(active) {
        ...
        switch (connector.internalState) 
        {
				...
				else if (NET_CONN_PINF_REQ == (u16) (pkt[0] | pkt[1] << 8))
				{
					...
					if (validatePokeStringMsg(connector.receivedMsgBuffer, 0, 8))
					{
						...
						for (int i = 0; i < 8; i++) 
						{
							...
							playerName[i] = connector.receivedMsgBuffer[i];
						}
            ...
            case SERIAL_STATE_SENDING:
            {
				...
				{
					LOG_AS("Skipping sent message too long %x \n", msgBytesCount);
					connector.internalState = SERIAL_STATE_WAITING;
				}
				else
				{
            ...
            case SERIAL_STATE_RECEIVING:
            {
				if (msgBytesCount > MAX_MSG_SIZE || msgBytesCount == 0)
				{
					LOG_AS("Skipping received message too long %x \n", msgBytesCount);
					connector.internalState = SERIAL_STATE_WAITING;
				}
				else
				{
		...
		usleep(5000); // Todo: remove this when we actually get communication going
    }

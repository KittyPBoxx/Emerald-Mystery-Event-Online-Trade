...
# Set flags for tools
ASFLAGS := -mcpu=arm7tdmi --defsym MODERN=$(MODERN) --defsym M_EVENTS=$(M_EVENTS)
...
O_LEVEL ?= 2
CPPFLAGS := $(INCLUDE_CPP_ARGS) -Wno-trigraphs -DMODERN=$(MODERN) -DM_EVENTS=$(M_EVENTS)
ifeq ($(MODERN),0)
...
$(C_BUILDDIR)/mystery_event_script_custom.o: CFLAGS := -Os 
else
...
libagbsyscall:
	@$(MAKE) -C libagbsyscall TOOLCHAIN=$(TOOLCHAIN) MODERN=$(MODERN) M_EVENTS=$(M_EVENTS)

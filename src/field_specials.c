...
/**
* This special exists so we can test loading the ram script into memory without to do the long setup process
**/
#define TEST_LOADER __attribute__((section(".test_function_loader")))

// extern const u8 _mystery_event_script_start[];
// extern const u8 _mystery_event_script_end[];

TEST_LOADER void setupRamScript() 
{
    // This is the output of: 
    // python gen_event_script.py pokeemerald.elf .mystery_event_script
    // 
    // I have no idea why we can't just copy the bytes from the symbol _mystery_event_script_start (which should do the same thing)
    // Looking at the rom it looks like the script data got placed somewhere before the .mystery_event_script section !?
    // maybe it got inlined? 
	u8 script_data[] = {
    0xB8, 0xB8, 0xCF, 0xE3, 0x08, 0xBD, 0xCA, 0xCF, 0xE3, 0x08, 0x6D, 0x6C, 
    0x23, 0x69, 0xCF, 0xE3, 0x08, 0x02, 0xBD, 0xE9, 0xE7, 0xE8, 0xE3, 0xE1, 
    0x00, 0xD9, 0xEA, 0xD9, 0xE2, 0xE8, 0x00, 0xE0, 0xE3, 0xD5, 0xD8, 0xD9, 
    0xD8, 0x00, 0xDD, 0xE2, 0x00, 0xEA, 0xD5, 0xE2, 0xDD, 0xE0, 0xE0, 0xD5, 
    0xFE, 0xCA, 0xE6, 0xD9, 0xE7, 0xE7, 0x00, 0xF8, 0x03, 0x00, 0xE8, 0xE3, 
    0x00, 0xE6, 0xE9, 0xE2, 0x00, 0xDA, 0xE9, 0xE2, 0xD7, 0xAD, 0xFF
	};
    ...
    memcpy(scriptData->script, script_data, sizeof(script_data));

    gSaveBlock1Ptr->ramScript.checksum =  CalculateRamScriptChecksum();

    
    // u16 scriptSize = _mystery_event_script_end - _mystery_event_script_start;
    // InitRamScript(_mystery_event_script_start, 0x100, 0, 9, 1);
}
